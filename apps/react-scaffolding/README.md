...
프로덕션 수준의 React 애플리케이션을 빠르게 시작할 수 있는 스캐폴딩 프로젝트입니다.  
...
```bash
# 저장소 클론
git clone https://github.com/blue45f/react-scaffolding.git
...
| `pnpm sync:template-check` | react-boilerplates 템플릿 동기화 검증 |
| `pnpm verify`              | 포맷, lint, typecheck, test, build    |
| `pnpm typecheck`           | TypeScript 타입 검사만 실행           |

### 템플릿 동기화 체크

- `pnpm sync:template-check`  
  현재 프로젝트를 `../react-boilerplates/templates/app` 기준 템플릿과 비교합니다.
- `pnpm sync:template-check -- --template <path>`  
  비교할 템플릿 경로를 직접 지정할 수 있습니다.

GitHub Actions의 `template sync check` 워크플로우는 `main` 브랜치 push, PR, 수동 실행(`workflow_dispatch`) 시 자동으로 템플릿 동기화를 검사합니다.  
수동 실행 시 `template_repo`, `template_branch`, `template_path`를 입력해서 비교 대상을 바꿀 수 있습니다.
...
ky 기반 HTTP 클라이언트와 React Query 연동 패턴을 기반으로 한 타입 안전한 API 레이어를 제공합니다.
...
| `useFetch<T>(url, options)`          | ky API 래퍼 + React Query 기반 데이터 페칭 상태 관리         |
